...
case class MatrixMultiplicationCoreConfigs(
    ...
    memSize: Int = 0x1000,
    addrWidth: Int = 32,
    rowAddrDiffA: Int = 16, // matSize * (matDWidth / 8)
    rowAddrDiffB: Int = 16, // matSize * (matDWidth / 8)
    rowAddrDiffC: Int = 16 * 4, // matSize * (resDWidth / 8)
  ...
  switch(state) {
    ...
    is(load) {
      ...
      when(loadState === loadRow) {
        memoryModule.io.address := io.baseAddressA + currentRow * cfg.rowAddrDiffA.U + index
        ...
        when(waitMem) {
          ...
          if (cfg.debugEnable) {
            printf(
              ...
              index,
              io.baseAddressA,
              io.baseAddressA + currentRow * cfg.rowAddrDiffA.U + index,
      ...
      when(loadState === loadCol) {
        memoryModule.io.address := io.baseAddressB + index * cfg.rowAddrDiffB.U + currentCol
        ...
        when(waitMem) {
          ...
          if (cfg.debugEnable) {
            printf(
              ...
              index,
              io.baseAddressB,
              io.baseAddressB + index * cfg.rowAddrDiffB.U + currentCol,
              memoryModule.io.dataOut
            )
          }
          when(index === (cfg.matSize - 1).U) {
    ...
    is(store) {
      ...
      memoryModule.io.address := io.baseAddressC + currentRow * cfg.rowAddrDiffC.U + currentCol * 4.U
      ...
      if (cfg.debugEnable) {
        printf(
          ...
          currentCol,
          io.baseAddressC,
          io.baseAddressC + currentRow * cfg.rowAddrDiffC.U + currentCol * 4.U,
