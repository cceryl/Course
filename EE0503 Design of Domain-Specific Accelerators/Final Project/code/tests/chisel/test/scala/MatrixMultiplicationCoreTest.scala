  ...
  "MatrixMultiplicationCore test" should "pass" in {
    ...
    test(
      ...
      for (testID <- 0 until 4) {
        ...
        dut.io.rowAddrDiffA.poke(testConfig.matSize.U)
        dut.io.rowAddrDiffB.poke(testConfig.matSize.U)
        dut.io.rowAddrDiffC.poke((testConfig.matSize * 4).U)
        ...
        for (mat <- 0 until matCount) {
          ...
          for (i <- 0 until matSize) {
            for (j <- 0 until matSize) {
              ...
              dut.io.address.poke(
                (baseAddressA + i * testConfig.matSize + j).U
              )
              dut.clock.step(1)
            }
          }
          for (i <- 0 until matSize) {
            for (j <- 0 until matSize) {
              ...
              dut.io.address.poke(
                (baseAddressB + i * testConfig.matSize + j).U
              )
              dut.clock.step(1)
            }
          }
        ...
        for (i <- 0 until matSize) {
          for (j <- 0 until matSize) {
            dut.io.address.poke(
              (baseAddressC + i * testConfig.matSize * 4 + j * 4).U
            )
            dut.clock.step(1)
            println(
