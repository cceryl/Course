...
void mat_mult(const long *mat_l, const long *mat_r, int *result, const unsigned int N, const unsigned int K, const unsigned int M)
{
    unsigned int n, k, m;
...
void quantize(const int *tensor_in, long *tensor_q, const int scale_factor, const int scale_factor_inv, const unsigned int size)
{
    unsigned int i;
    ...
    for (i = 0; i < size; i++)
    {
        ...
        if (tensor_int > INT8_MAX_VALUE * scale_factor_inv)
            tensor_q[i] = (long)INT8_MAX_VALUE;
        ...
        else
        {
            ...
            tensor_q[i] = (long)rounded_value; /* store quantized value in output tensor */
        }
    }
}
