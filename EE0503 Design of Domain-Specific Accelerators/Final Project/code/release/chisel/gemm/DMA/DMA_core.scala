...
class StreamReader(nXacts: Int, outFlits: Int, maxBytes: Int)(implicit
    p: Parameters
) extends LazyModule {
...
class StreamReaderCore(nXacts: Int, outFlits: Int, maxBytes: Int)(implicit
    p: Parameters
) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    xactBusy := (xactBusy | Mux(tl.a.fire, xactOnehot, 0.U)) &
      ~Mux(tl.d.fire && edge.last(tl.d), UIntToOH(tl.d.bits.source), 0.U)
    ...
    tl.a.bits := edge
      .Get(fromSource = xactId, toAddress = sendaddr, lgSize = reqSize)
      ._2
...
class StreamWriter(nXacts: Int, maxBytes: Int)(implicit p: Parameters)
    extends LazyModule {
  val node = TLClientNode(
    Seq(
      TLMasterPortParameters.v1(
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val wmask = Cat(
      (0 until beatBytes)
        .map(i => (i.U >= byteOff) && (i.U < bytesToSend))
        .reverse
    )
