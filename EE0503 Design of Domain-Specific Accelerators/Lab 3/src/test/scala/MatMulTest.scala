    ...
    test(new MM_TOP(cfg = test_cfg)) { dut =>
      ...
      for (testID <- 0 until 10) {
        ...
        val MatA = generateRandomMatrixUnSigned(
          rows = test_cfg.matSize,
          cols = test_cfg.matSize,
        ...
        for (row <- 0 until test_cfg.matSize) {
          ...
          for (col <- 0 until test_cfg.matSize) {
            val addr_in_mem =
              test_cfg.matA_baseAddr + row * test_cfg.matSize + col
        ...
        for (row <- 0 until test_cfg.matSize) {
          ...
          for (col <- 0 until test_cfg.matSize) {
            val addr_in_mem =
              test_cfg.matB_baseAddr + row * test_cfg.matSize + col
        ...
        for (row <- 0 until test_cfg.matSize) {
          for (col <- 0 until test_cfg.matSize) {
            val addr_in_mem =
              test_cfg.matC_baseAddr + row * test_cfg.matSize + col
            dut.io.addr.poke(addr_in_mem.U)
            dut.io.writeEn.poke(false.B)
            dut.io.enable.poke(true.B)
            dut.clock.step(1)
